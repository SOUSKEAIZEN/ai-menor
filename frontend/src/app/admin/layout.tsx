  ...
  return (
    <div className="flex h-[100dvh] bg-[var(--background)] text-[var(--main)] ambient-bg p-3 pt-safe md:p-4 gap-4">
          ...
          {sidebarLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                <Icon className={`w-5 h-5 transition-transform duration-400 ${isActive ? "text-[var(--main)] scale-110" : "text-[var(--muted)] group-hover:text-[var(--main-light)] group-hover:scale-110"}`} />
                {link.label}
              </Link>
            );
          })}
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full relative z-10 h-full gap-4">
        {/* Floating Header */}
        <header className="h-16 shrink-0 relative floating-header flex items-center justify-between md:justify-end px-4 md:px-6 z-20">
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar rounded-3xl bg-[var(--glass-fill)] backdrop-blur-2xl border border-border/40 shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
